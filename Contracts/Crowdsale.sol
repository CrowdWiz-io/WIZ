...
contract Crowdsale is Owned {
    ...
    uint constant REQUIRED_CONFIRMATIONS = 2;

    address[] confirmOwners = [
        0xa0d355473e1454bbfe6e791b61aaeabccc38653f,
        0xc11ac8f445331141bdf4702e9e75171bc18b4c8f
    ];
    mapping(address => bool) isConfirmOwner;
    mapping (uint => Transaction) transactions;
    uint transactionCount;

    event Transfer(address indexed _to, uint _value);
    event NewTransaction(uint indexed _id, address indexed _destination, uint _value);
    ...
    modifier onlyConfirmOwner(){
        require(isConfirmOwner[msg.sender]);
        _;
    }
    ...
    modifier confirmed(uint transactionId, address _owner) {
        require(transactions[transactionId].confirmations[_owner]);
        _;
    }
    ...
    function Crowdsale(address _beneficiary){
        ...
        for(uint i = 0; i < confirmOwners.length; i++){
            isConfirmOwner[confirmOwners[i]] = true;
        }
    }
    ...
    function mintPartners()
    public
    only(owner)
    {
        _mint(0x3edf937cb9f0f7c3b3a0bede2ad8bc08b6babc79, 4 * (10**24)); //Reserve
        _mint(0x3edf937cb9f0f7c3b3a0bede2ad8bc08b6babc79, 4 * (10**24)); //Develop
        _mint(0x3edf937cb9f0f7c3b3a0bede2ad8bc08b6babc79, 6 * (10**24)); //Investors
    }
    ...
    salesRunning
    {
        ...
        NewTransaction(transactionId, _to, _amount);
    }
    ...
    returns (bool)
    {
        uint count = 0;
        for (uint i = 0; i < confirmOwners.length; i++) {
            if (transactions[transactionId].confirmations[confirmOwners[i]])
            count += 1;
    ...
    notExecuted(transactionId)
    {
        if (isConfirmed(transactionId)) {
            ...
            require(totalSaleSupply.add(tx.value) <= MAX_SALE_SUPPLY);

            totalSaleSupply = totalSaleSupply.add(tx.value);

            _mint(tx.destination, tx.value);
            Transfer(tx.destination, tx.value);
        }
    }
